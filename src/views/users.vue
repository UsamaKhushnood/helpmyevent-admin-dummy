  ...
  <div>
    ...
    <CDataTable class="mt-6" :fields="fields" :items="data.items" :total="data.total" :loading="loading"
      ...
      <template #status="{ data }">
        <StatusBadge v-if="data.status" :status="data.status"> </StatusBadge>
      </template>
      <template #is_profile_completed="{ data }">
        <StatusBadge v-if="data.is_profile_completed === 1" status="success">
          Completed
        </StatusBadge>
          ...
          <div class="rounded-lg border bg-background p-4 flex items-center justify-between"
            ...
            <div class="flex items-center gap-3">
              ...
              <div>
                ...
                <h3 class="text-xs mt-0.5">
                  {{ $filters.formatDate(file.created_at) }}
                </h3>
              </div>
            </div>
  ...
  data() {
    return {
      ...
      fields: [
        ...
        { key: "is_profile_completed", label: "Profile Completed", sortable: true },
        { key: "status", label: "Status", collapse: true, sortable: true },
