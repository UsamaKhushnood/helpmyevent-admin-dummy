<template>
  <div class="select-none w-full">
    <Select v-model="modelValue" :disabled="loading || disabled">
      <SelectTrigger ref="trigger" class="w-full bg-background cursor-pointer" :class="triggerClass">
        <div class="flex items-center truncate w-full">
          ...
          <slot name="selected" :placeholder="placeholder" :value="modelValue">
            <SelectValue class="truncate" :placeholder="placeholder"></SelectValue>
      ...
      <SelectContent :style="{ maxWidth: inputWidth }">
        <SelectGroup>
          <SelectLabel v-if="!loading && !options?.length"
            class="py-20 text-center text-base font-normal text-muted-foreground">No Options
            Available
          </SelectLabel>
          <SelectLabel v-else class="text-xs text-muted-foreground">{{ placeholder }}</SelectLabel>
          <template v-if="!loading">
            <slot>
              <SelectItem v-for="(option, index) in options" :key="option.value" :value="option.value"
                :disabled="option.disabled">
                <slot name="item" :option="option">
                  <div class="flex items-center gap-2">
                    <img v-if="option.img" :src="option.img" class="w-[20px]" alt="icon" aria-hidden="true" />
                    <component :is="option.icon" v-if="option.icon" alt="icon" aria-hidden="true" />
                    {{ option.label }}
                  </div>
                </slot>
              </SelectItem>
