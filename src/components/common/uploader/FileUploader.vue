<template>
  <div>
    <slot :addListeners="addListeners" :deleteFile="deleteFile" :dragOver="dragOver" :edit="edit" :error="error"
      :files="files" :formatFileSize="formatFileSize" :getFilePreview="getFilePreview"
      :handleDragLeave="handleDragLeave" :handleDragOver="handleDragOver" :handleDrop="handleDrop"
      :handleFileSelect="handleFileSelect" :handleUpload="handleUpload" :readers="readers" :reset="reset"
      :resize="resize" :triggerFileInput="triggerFileInput" :uniqueID="uniqueID">
      <DragnDropArea v-if="files.length <= 0 && !modelValue" :description="description" :dragOver="dragOver"
        :vertical="vertical">
      </DragnDropArea>

      <template v-for="(reader, readerIndex) in readers" :key="readerIndex">
        <UploadingInfo :class="{ hidden: reader?.showPreview }" :currentProgress="reader?.currentProgress"
          :file="files[readerIndex]" :index="readerIndex" :totalSize="reader?.totalSize" />
        <FilePreview :file="files[readerIndex]" :index="readerIndex" :showPreview="reader?.showPreview" />
      </template>

      <UploadErrorMessage :message="error.message" />
    </slot>

    <input :id="uniqueID" :ref="uniqueID" :accept="accept" :multiple="multiple" aria-hidden="true" class="hidden"
